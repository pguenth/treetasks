...
\fi
    ...
    % upper row
    \X=0\unit
    \longfkeyd      (2\base - \unit) {Tab}{switch tab}
    \keyd          Q{quit},q{close tab}
...
\fi
    ...
    \keyd           A{new child (bottom)},a{edit field\note{4}}
    \keyd           S{save},s{sort\note{5}}
    \keyd          D{cut},d{cut\\delete\note{2}}
    ...
    \keyd          C{replace title},c{replace field\note{3}}
    \keyd           V{toggle show cancelled},v{toggle cancelled}
    \keyd          B{},b{toggle flat tree}
...
\def\blockB{
    \makeblock{6cm}{Delete fields \note{2}}{
        Keychains for deleting tasks and fields:
        \begin{itemize}
            \item \shortcut{dd} Cut cursor
            \item \shortcut{dD} Delete cursor (while keeping clipboard)
            \item \shortcut{dc} Delete category 
            \item \shortcut{dp} Delete priority
            \item \shortcut{dr} Delete due date
            \item \shortcut{de} Delete scheduled date
            \item \shortcut{dt} Delete text 
        \end {itemize}
        \textbf{Watch out:} no operations have undo at the moment
   }
}
\def\blockC{
    \makeblock{6cm}{Sort \note{5}}{
...
\def\blockD{
    \makeblock{6cm}{Replace/Change \note{3}}{
        Keychains for editing tasks by replacing fields.
        \begin{itemize}
            \item \shortcut{C} Replace title
            \item \shortcut{cp} Replace priority
            \item \shortcut{cc} Replace category
            \item \shortcut{cr} Replace due date
            \item \shortcut{ce} Replace scheduled date
            \item \shortcut{ct} Replace text
...
\def\blockE{
    \makeblock{6cm}{Edit \note{4}}{
        Keychains for editing tasks by replacing fields.
        \begin{itemize}
            \item \shortcut{at} or \shortcut{aa} Edit text
            \item \shortcut{ap} Edit priority
            \item \shortcut{ac} Edit category
            \item \shortcut{ar} Edit due date
            \item \shortcut{ae} Edit scheduled date
        \end {itemize}
        Without using keychains, the title can be edited:
        \begin{itemize}
...
\Shortunderstack{ {\protect\blockD}  {\protect\blockE}  }
\Shortunderstack{ {\protect\blockC}  }
\Shortunderstack{ {\protect\blockF}  }
\Shortunderstack{ {\protect\blockG}  }
